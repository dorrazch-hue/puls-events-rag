...
def extraire_texte(champ):
    """Extrait le texte francais d un champ multilingue."""
    if isinstance(champ, dict):
        return champ.get('fr', '') or champ.get('en', '') or ''
    return champ or ''

def nettoyer_evenement(e):
    """Transforme un evenement brut en version propre."""
...
def filtrer_evenements(evenements):
    """Garde les evenements de moins d un an dans le Grand Paris."""
    resultats = []
    for e in evenements:
...
if __name__ == "__main__":
    ...
    print(f"Evenements bruts : {len(evenements)}")
    filtres = filtrer_evenements(evenements)
    print(f"Apres filtre (moins d un an, Grand Paris) : {len(filtres)}")
    ...
    print("Donnees nettoyees sauvegardees dans data/events_clean.json")
